...
// We find the second match of targetStart
let index = code.indexOf(targetStart);
index = code.indexOf(targetStart, index + 1); // Skip the first one we already patched
...
if (index > -1) {
    ...
    const endIndex = code.indexOf(targetEnd, index) + targetEnd.length;
    ...
    if (endIndex > -1) {
        ...
        console.log('UI Patched successfully 2nd time!');
    } else {
        console.log('End index not found near index: ' + index);
    }
} else {
    console.log('Could not find second target block string.');
}
